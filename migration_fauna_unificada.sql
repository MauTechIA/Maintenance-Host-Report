...
-- Objetivos:
-- 1) Guardar datos operativos (no sólo PDF).
-- 2) Separar zona (área aeroportuaria) de ubicación geográfica (coordenadas).
-- 3) Eliminar tablas legacy no usadas por el frontend actual.
...
-- Opcional: habilitar RLS y políticas básicas abiertas (igual que modelo anterior)
alter table public.fauna_reports enable row level security;

drop policy if exists fauna_reports_select on public.fauna_reports;
drop policy if exists fauna_reports_insert on public.fauna_reports;
drop policy if exists fauna_reports_update on public.fauna_reports;
drop policy if exists fauna_reports_delete on public.fauna_reports;

create policy fauna_reports_select on public.fauna_reports for select using (true);
create policy fauna_reports_insert on public.fauna_reports for insert with check (true);
create policy fauna_reports_update on public.fauna_reports for update using (true);
create policy fauna_reports_delete on public.fauna_reports for delete using (true);
