...
## Gestión de Pistas con Supabase

La aplicación ahora carga las opciones de pista dinámicamente desde Supabase en lugar de tenerlas hardcodeadas.

### Crear tabla de pistas

1. Ve al **SQL Editor** en tu Dashboard de Supabase
2. Ejecuta el contenido del archivo `create_pistas_table.sql`
3. Esto creará la tabla `pistas` con las pistas iniciales
...
CREATE TABLE pistas (
    ...
    nombre VARCHAR(100) NOT NULL UNIQUE,     -- Nombre completo (ej: "RWY 04C-22C")
    codigo VARCHAR(20) NOT NULL UNIQUE,      -- Código corto (ej: "04C-22C")
    descripcion TEXT,                        -- Descripción opcional
    activo BOOLEAN DEFAULT true,             -- Para activar/desactivar pistas
...
- **Agregar nuevas pistas**: Inserta registros en la tabla `pistas`
- **Desactivar pistas**: Cambia `activo = false` en lugar de eliminar
- **Orden**: Las pistas se ordenan por `codigo` al cargar

### Políticas de seguridad

La tabla tiene RLS habilitado con política pública de lectura, permitiendo que cualquier usuario vea las pistas activas. Para restringir escritura, descomenta las políticas en el script SQL.